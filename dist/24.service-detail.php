<?php include_once "parts/head.php" ?>
<body>
<?php include_once "parts/header.php" ?>

<?php include_once "parts/breadcrumbs.php" ?>

<div class="container">
    <div class="row">
        <h1 class="page-title">Ремонт и обслуживание лазерных принтеров Canon в сервисном центре / в офисе (с выездом
            мастера)</h1>
    </div>
</div>

<div class="container">
    <div class="row">
        <div class="template-orders">
            <div class="text-section">
                <img src="img/jpeg_files/r-d.jpg" alt="image" align="left">

                <p>Компания «ДСМ-Трейд» является поставщиком специализированных IT-решений в области автоматизации
                    проектирования, подготовки производства и документооборота для авиастроения, автомобилестроения,
                    судостроения, полиграфии, промышленного и гражданского строительства и многих других отраслей
                    промышленности. Мы занимаемся автоматизацией проектно-конструкторских и технологических работ,
                    дистрибьюцией, системной интеграцией программного и аппаратного обеспечения для государственных
                    структур, банков, промышленных предприятий и проектных организаций.
                </p>

                <p> Компания представляет на украинском рынке крупнейших мировых производителей аппаратного обеспечения
                    – широкоформатные и производительные системы Océ, профессиональные МФУ и цифровые печатные машины
                    формата А3 Canon и Develop, струйные плоттеры Canon и Epson, широкоформатные сканеры Océ и
                    Colortrac, линейно-матричные принтеры Printronix, конвертовальное оборудование Neopost.
                </p>

                <p>Специалисты «ДСМ-Трейд» не только проведут анализ эффективности работы вашего предприятия, но
                    подберут оптимальное решение с учетом стоящих задач и помогут внедрить и адаптировать системы под
                    конкретные задачи.
                </p>
                <p>
                    Для бесперебойной работы поставляемого оборудования компанией «ДСМ-Трейд» была разработана
                    комплексная система технической поддержки, отвечающая европейским стандартам качества. Наша
                    сервисная служба готова взять на себя обязанности по своевременному предупреждению отказов
                    оборудования и устранению возникших неполадок за минимально короткий срок.
                </p>
            </div>

            <div class="clearfix"></div>


            <div class="gray-form">
                <div class="template-orders__form">
                    <h2 class="template-orders__form--title text-uppercase">Подать заявку на обслуживание</h2>

                    <p class="template-orders__form--subtitle">Напишите название услуги или задайте Ваш вопрос</p>

                    <div class="template-orders__form--wrapper">
                        <form action="#">
                            <fieldset>
                                <label for="gf-1" class="custom-input-label">Ваше имя
                                    <mark class="red-marker">*</mark>
                                </label>
                                <input type="text" id="gf-1" class="custom-input" placeholder="Введите ваше имя">
                            </fieldset>
                            <fieldset>
                                <label for="gf-2" class="custom-input-label">E-mail</label>
                                <input type="email" id="gf-2" class="custom-input" placeholder="Введите e-mail адрес">
                            </fieldset>
                            <fieldset>
                                <label for="gf-4" class="custom-input-label">Телефон
                                    <mark class="red-marker">*</mark>
                                </label>
                                <input type="tel" id="gf-4" class="custom-input" placeholder="Введите номер телефона">
                            </fieldset>
                            <fieldset>
                                <label for="gf-3" class="custom-textarea-label">Какая услуга вас интересует
                                    <mark class="red-marker">*</mark>
                                </label>
                            <textarea name="" id="gf-3" class="custom-textarea" cols="30" rows="6"
                                      placeholder="Напишите название услуги или задайте Ваш вопрос"></textarea>
                            </fieldset>
                            <button type="submit" class="btn btn-link btn-round btn--blue">Отправить</button>
                        </form>
                    </div>

                </div>
            </div>
        </div>

    </div>
</div>

<?php include_once "parts/footer.php" ?>
</body>
</html>
